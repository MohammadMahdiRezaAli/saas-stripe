...
export default function TestimonialMultiple() {
  return (
    <div className="relative w-full bg-[#1B1340] py-16 lg:py-20">
      {/* Full-width animated background overlays */}
      <div
        className="absolute inset-0 -z-10 overflow-hidden opacity-30 blur-3xl"
        aria-hidden="true"
      >
        <div
          className="w-full h-full bg-gradient-to-tr from-[#5C3DFF] to-[#8A66FF]"
          style={{
            clipPath:
      ...
      <div className="max-w-[100%] px-4 md:px-8 lg:px-16">
        <div className="mx-auto grid max-w-2xl grid-cols-1 gap-8 text-sm leading-6 text-[#E0E0F1] sm:grid-cols-2 xl:grid-flow-col xl:grid-cols-4">
          ...
          <figure className="rounded-2xl bg-[rgba(92,61,255,0.1)] shadow-lg ring-1 ring-[#5C3DFF]/20 sm:col-span-2 xl:col-start-2 xl:row-end-1">
            ...
            <figcaption className="flex items-center gap-x-4 border-t border-[#8A66FF]/20 px-6 py-4">
              <div>
                  ...
                  {column.map((testimonial) => (
                    ...
                    >
                      <blockquote className="text-[#E0E0F1]">
                        ...
                        {testimonial.icon && (
                          <div className="mt-6 flex items-center gap-x-4">{testimonial.icon}</div>
                        )}
                      </blockquote>
